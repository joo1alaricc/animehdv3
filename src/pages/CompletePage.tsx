...
const CompletePage = () => {
  ...
  const mapApiData = (data: any[]): Anime[] => {
    ...
    return data.map((item: any) => ({
      ...
      genre: ['Action'],
      synopsis: `Completed on ${item.lastReleaseDate || 'recently'}.`,
    ...
    const fetchComplete = async () => {
      try {
        ...
        if (json.status === 'success' && json.data) {
          setCompleteList(mapApiData(json.data.animeList));
  ...
  if (loading) return <Loader message="RETRIVING ARCHIVED DATA..." />;

  return (
    <div className="max-w-7xl mx-auto p-4 space-y-12">
      <header className="bg-[#4CD964] border-8 border-black p-8 md:p-12 shadow-[12px_12px_0px_0px_rgba(0,0,0,1)] transform rotate-1 relative overflow-hidden">
        <div className="absolute top-0 right-0 opacity-10 font-black text-8xl -translate-y-4">COMPLETE</div>
        <h1 className="text-4xl md:text-7xl font-black oswald text-black italic relative z-10">COMPLETE SERIES</h1>
        <p className="text-lg md:text-2xl font-bold oswald text-white bg-black px-2 py-1 inline-block mt-4 relative z-10">BINGE-WORTHY CLASSICS</p>
      </header>
      ...
      <div className="flex justify-center gap-4 pb-10">
        <button 
          ...
          disabled={page === 1}
          className={`px-8 py-3 font-black oswald border-4 border-black shadow-[4px_4px_0px_0px_black] active:shadow-none active:translate-x-1 active:translate-y-1 transition-all ${page === 1 ? 'bg-gray-300 cursor-not-allowed' : 'bg-white hover:bg-[#FFCC00] text-black'}`}
        >
          PREV
        </button>
